...
export default {
  snipeVanityUrl: async (vanity_url) => {
    return new Promise(async (resolve, reject) => {
      const send = await fetch(
        `https://discord.com/api/v8/guilds/${config.sniperGuild}/vanity-url`,
        {
          method: "PATCH",
          headers: {
  ...
  leaveGuild: async (guild_id) => {
    return new Promise(async (resolve, reject) => {
      const send = await fetch(
        `https://discord.com/api/v8/users/@me/guilds/${guild_id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  joinGuild: async (vanity_url) => {
    return new Promise(async (resolve, reject) => {
      const send = await fetch(
        `https://discord.com/api/v8/invites/${vanity_url}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
